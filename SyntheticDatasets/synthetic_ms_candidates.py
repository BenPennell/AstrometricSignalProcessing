...
def solve_single(ra, dec, pmra, pmdec, plx, gmag, return_fits=False):
    t = get_gost_mod(ra, dec)
    return gw.rapid_single_star(ra, dec, pmra, pmdec, plx, gmag, t, return_fits=return_fits)
...
def create_synthetic_data(object_count, catalogue,
    ...
    with tqdm_joblib(pbar if verbose else tqdm(disable=True)):
        results_single = Parallel(
            ...
            delayed(solve_single)(ra[i], dec[i], pmra[i], pmdec[i], plx[i], gmag[i], return_fits=return_fits) for i in sin_idx
        )
